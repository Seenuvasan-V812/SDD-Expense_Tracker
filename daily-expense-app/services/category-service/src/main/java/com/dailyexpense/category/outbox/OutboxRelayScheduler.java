...
/**
 * T096 — Category-service outbox relay.
 * Polls category_db.outbox WHERE published=false → publishes EventEnvelope to Kafka → marks published.
 */
@Component
...
public class OutboxRelayScheduler extends AbstractOutboxRelayScheduler {
    ...
    public OutboxRelayScheduler(
            ...
            @Value("${outbox.relay.topic:categories}") String topic,
            @Value("${spring.application.name:category-service}") String producer) {
