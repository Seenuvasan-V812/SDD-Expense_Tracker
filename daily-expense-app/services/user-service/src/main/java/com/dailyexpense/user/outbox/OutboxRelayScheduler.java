...
/**
 * T096 — User-service outbox relay.
 * Polls user_db.outbox WHERE published=false → publishes EventEnvelope to Kafka topic → marks published.
 */
@Component
...
public class OutboxRelayScheduler extends AbstractOutboxRelayScheduler {
    ...
    public OutboxRelayScheduler(
            ...
            @Value("${outbox.relay.topic:users}") String topic,
            @Value("${spring.application.name:user-service}") String producer) {
