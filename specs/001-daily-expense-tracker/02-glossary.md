...
| **Source Inputs** | `Daily expense tracker Requirements - updated.md`, `.specify/memory/constitution.md` (v1.1.2), `01-context-specification.md` |
...
| **deliveryRef** | A secure, opaque reference string produced by `user-service` and passed to the **Notification** context via the `SecureNotificationDeliveryPort` (ACP). It encapsulates the identity and expiry of a time-sensitive token (email-verification link, password-reset link) without exposing the raw token across a service boundary. The Notification service resolves the actual delivery URL from it. (DEF-001) | ❌ `tokenRef`, `notificationToken`, `deliveryToken`, `linkRef` | Identity & Access (defined) → Notification (consumed) |
...
| **ExpensePrototype** | A **Value Object** embedded in a **Recurring Expense** aggregate that stores the template values — amount, category, payment method, merchant, description, notes, and tags — copied verbatim into each generated **Occurrence**. Changing the ExpensePrototype affects only future occurrences, not past ones. (Doc 09 §4.5 carries `merchant`, `notes`, `recurring_expense_tags`.) | ❌ `Template` (bare), `RecurringExpenseTemplate`, `Blueprint`, `Pattern`, `Schema` | Expense / Transaction |
| **FiredThresholdSet** | The `Set<BudgetThreshold>` stored inside a `BudgetPeriodLedger` tracking which threshold levels (`EIGHTY_PERCENT`, `EXCEEDED`) have already fired in the current **Budget Period**. A threshold absent from this set may fire; one already present will not re-alert (BUD-INV-5, Doc 06 §4.1). The set is reset to empty when a new period opens (BUD-INV-8). Persisted in `budget_db` as the two boolean `fired_*` columns. (DEF-002) | ❌ `alertedThresholds`, `triggeredSet`, `firedAlerts`, `thresholdLog` | Budget |
...
| **PeriodWindow** | A Value Object `{ startDate: DateOnly, endDate: DateOnly }` embedded in each `BudgetPeriodLedger` that defines the calendar boundary of one **Budget Period**. Derived from the `BudgetPeriodType` (WEEKLY / MONTHLY) and the Budget's anchor date. Used to scope `RecordSpendingObservationCommand` inputs (Doc 06 §4.4) and to determine rollover eligibility. (DEF-003) | ❌ `DateRange`, `PeriodRange`, `BudgetWindow`, `TimeWindow` | Budget |
...
| **RecurrenceBound** | A Value Object embedded in a **Recurring Expense** template that optionally limits how long the template generates **Occurrences**. Takes one of two forms: `{ endDate: DateOnly }` (generate until a calendar date) or `{ maxOccurrences: int }` (generate at most N occurrences). If absent, the template runs indefinitely. Persisted as `recurring_expenses.end_date` / `recurring_expenses.max_occurrences` (Doc 09 §4.5). (DEF-004) | ❌ `EndBound`, `RecurrenceLimit`, `TerminationRule`, `StopCondition` | Expense / Transaction |
